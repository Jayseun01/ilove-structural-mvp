...
def text_point(entity):
    try:
        if entity.dxftype() == "TEXT":
            halign = int(dxf_get(entity, "halign", 0) or 0)
            valign = int(dxf_get(entity, "valign", 0) or 0)
            if halign != 0 or valign != 0:
                point = point_tuple(dxf_get(entity, "align_point", None))
                if point:
                    return point
    except Exception:
        pass
...
def text_alignment(entity):
    try:
        typ = entity.dxftype()
    except Exception:
        return "start", "alphabetic"

    if typ == "MTEXT":
        try:
            attachment = int(dxf_get(entity, "attachment_point", 1) or 1)
        except Exception:
            attachment = 1

        anchor = {
            1: "start", 4: "start", 7: "start",
            2: "middle", 5: "middle", 8: "middle",
            3: "end", 6: "end", 9: "end",
        }.get(attachment, "start")

        baseline = {
            1: "hanging", 2: "hanging", 3: "hanging",
            4: "middle", 5: "middle", 6: "middle",
            7: "text-after-edge", 8: "text-after-edge", 9: "text-after-edge",
        }.get(attachment, "hanging")
...
def text_bbox(x, y, text, height, rotation, anchor="start", baseline="alphabetic"):
    ...
    local_corners = [(left, bottom), (left + width, bottom), (left + width, top), (left, top)]
    bboxes = []
    for dx, dy in local_corners:
        rx, ry = rotate_xy(dx, dy, rotation)
        bboxes.append(point_bbox(float(x) + rx, float(y) + ry, 0.0))
...
def entity_bbox(entity):
    try:
        ...
        if typ in {"TEXT", "MTEXT"}:
            ...
            anchor, baseline = text_alignment(entity)
            return text_bbox(x, y, text, height, rotation, anchor, baseline)
...
def entity_to_record(entity, source_type="", source_layer=""):
    ...
    try:
        if typ == "LINE":
            ...
            record["text_anchor"], record["text_baseline"] = text_alignment(entity)
        elif typ == "DIMENSION":
            measurement = dimension_measurement(entity)
...
def svg_text_element(x, y, text, color, font_size, rotation=0.0, anchor="start", baseline="alphabetic"):
    clean = svg_escape(text)[:140]
    rotate = float(rotation or 0.0) % 360.0
    if rotate > 180.0:
        rotate -= 360.0
    transform = ""
    ...
    return (
        ...
        f'text-anchor="{anchor}" dominant-baseline="{baseline}"{transform}>{clean}</text>'
    )
                ...
                if text:
                    ...
                    parts.append(
                        svg_text_element(
                            ...
                            anchor=record.get("text_anchor", "start"),
                            baseline=record.get("text_baseline", "alphabetic"),
                        )
                    )
            elif typ == "DIMENSION":
