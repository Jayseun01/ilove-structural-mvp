    ...
    if uploaded_file:
        ...
        try:
            ...
            if st.button("🔥 Purge and Prepare Download"):
                deleted = purge_layers_from_modelspace(doc, keep)
                data = write_doc_to_temp_bytes(doc)

                st.success(f"Deleted {deleted} modelspace entities.")
    ...
    if st.session_state.docs_loaded:
        ...
        if st.session_state.prepared:
            ...
            st.download_button(
                ...
                st.write("#### Rejected Reference Markers")
                st.dataframe(
                    arch_det.get("rejected_markers", []),
                    use_container_width=True,
                )

                st.write("#### Rejected Target Markers")
                st.dataframe(
                    struc_det.get("rejected_markers", []),
                    use_container_width=True,
                )
                ...
                for m in struc_det.get("trusted_markers", [])[:300]:
                    sample.append({
                        ...
                        "circle_radius": round(m.get("circle_radius", 0), 3),
                        "circle_source": m.get("circle_source"),
                        "circle_handle": m.get("circle_handle"),
                    })

                st.dataframe(sample, use_container_width=True)

                st.write("#### Regions After Interior/Detail Filter")
                ...
                for r in st.session_state.regions:
                    region_diag.append({
                        "region": r.get("name"),
                        "sync_markers_after_filter": len(r.get("markers", [])),
                        "all_detected_markers_before_filter": len(
                            r.get("all_markers", r.get("markers", []))
                        ),
                        "interior_markers_ignored": r.get("interior_marker_count", 0),
                        "bbox_min_x": round(r.get("bbox", {}).get("min_x", 0), 3),
                        "bbox_max_x": round(r.get("bbox", {}).get("max_x", 0), 3),
                        "bbox_min_y": round(r.get("bbox", {}).get("min_y", 0), 3),
                        "bbox_max_y": round(r.get("bbox", {}).get("max_y", 0), 3),
                    })

                st.dataframe(region_diag, use_container_width=True)

                st.write("#### Axis Group Diagnostics")

                axis_diag = []

                for r in st.session_state.regions:
                    axis_groups = r.get("axis_groups", {})

                    for orientation, groups in axis_groups.items():
                        for i, g in enumerate(groups, start=1):
                            axis_diag.append({
                                "region": r.get("name"),
                                "orientation": orientation,
                                "axis_position": i,
                                "axis_label": g.get("label"),
                                "coord": g.get("coord"),
                                "marker_count": g.get("marker_count"),
                                "writable_marker_count": g.get("writable_marker_count"),
                                "label_count": g.get("label_count"),
                                "mixed_labels": ", ".join(g.get("mixed_labels", [])),
                                "label_counts": str(g.get("label_counts", {})),
                                "avg_confidence": g.get("avg_confidence"),
                                "min_confidence": g.get("min_confidence"),
                            })

                st.dataframe(axis_diag, use_container_width=True)
